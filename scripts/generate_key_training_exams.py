...
def validate_key_training_pool(questions: list[dict], domains: list[dict]) -> None:
    ...
    errors: list[str] = []
    if len(questions) < 10:
        errors.append(f"pool has {len(questions)} questions, need at least 10")
    for d in domains:
...
def main() -> int:
    ...
    for cat in KEY_TRAINING_CATEGORIES:
        ...
        if len(payload["questions"]) < 10:
            raise SystemExit(f"{cat.id}: only {len(payload['questions'])} scenario questions")
